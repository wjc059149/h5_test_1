...
#from common.special import merchant_login_id as merchant
#from common.special import random_32
    ...
    for i in range(num_rows):
        if sheet_page.row_values(i)[0] == case_name:
            ...
            for j in range(len_list):
                ...
                if cell_type == 1:
                    cell = cell.strip()  # 去掉字符串两边的空白符（包括'\n', '\r',  '\t',  ' ')
                    # cell = cell.strip(r"\n")
    ...
    for item in call_list:
        ...
        for i in lis:
            ...
            if 'response' in li[0]:
                ...
                li[3] = str(env)
            elif li[0].split('(')[0] in ('mgt', 'partner', 'merchant'):
                """
                例如mgt(username, password, 1)的li[0]为mgt(username，那么li[0].split('(')[0]为mgt
                """
                if env == 3:
