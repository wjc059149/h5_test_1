...
import readConfig
from framework import report_template
...
#nhuukqatlfaibcdc ；pyvqmwxdbllnbfbg ；lwgooveaeqetbfdd ；qsmwjbuuordbbaji 授权码


def send_email(upload_report=True, remote_path=None, result_path=None, summary_path=None, att=False):
    if summary_path is None:
        summary_path = base_page.new_html("summary")
    if result_path is None:
        result_path = base_page.new_html('report')  # 最新的详细测试结果文件路径
    ...
    context = MIMEText(mail_context, 'html', 'utf-8')#构建邮件内容
    ...
    # 构造附件1
    att1 = MIMEText(open(result_path, 'rb').read(), 'base64', 'utf-8') #构建邮件内容
    ...
    # 附件名称为中文时的写法
    att1.add_header("Content-Disposition", "attachment", filename=("utf-8", "", "电子发票H5的UI详细测试结果.html"))
    if att is True:
        message.attach(att1)
